...
addUser :: String -> IO ()
addUser userName = withConn "tools.db" $ \conn -> execute conn "INSERT INTO users (username) VALUES (?)" (Only userName) >> print "user added"
...
printAvailable :: IO ()
printAvailable = printToolQuery $ mconcat [ "SELECT * FROM tools ", "WHERE id NOT IN ", "(SELECT tool_id FROM checkedout);"]
